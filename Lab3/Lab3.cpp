...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DRAWITEM:
        ...
        if (lpInfo->CtlID == ID_BUTTON_1) {
            ...
            arr[2] = { width / 2 + width / 4, height / 2};
            arr[3] = { width / 2 - width / 4, height / 2 - height / 4 };
            HGDIOBJ original = NULL;
            original = SelectObject(lpInfo->hDC, GetStockObject(DC_PEN));
            SelectObject(lpInfo->hDC, GetStockObject(DC_PEN));
            SelectObject(lpInfo->hDC, GetStockObject(DC_BRUSH));
            SetDCBrushColor(lpInfo->hDC, RGB(255, 0, 0));
            SetDCPenColor(lpInfo->hDC, RGB(255, 0, 0));
            Polygon(lpInfo->hDC, arr, 4);
            SelectObject(lpInfo->hDC, original);
        }
        if (lpInfo->CtlID == ID_BUTTON_2) {
            RECT rcItem;
            rcItem = lpInfo->rcItem;
            FillRect(lpInfo->hDC, &rcItem, CreateSolidBrush(RGB(0, 255, 100)));
            POINT arr[4];
            INT width, height;
            width = rcItem.right - rcItem.left;
            height = rcItem.bottom - rcItem.top;
            RECT rcIm;
            rcIm = { width / 2 - width / 8, height / 2 - height / 4, width / 2 + width / 8, height / 2 + height / 4 };
            Rectangle(lpInfo->hDC, rcIm.left, rcIm.top, rcIm.right, rcIm.bottom);
            FillRect(lpInfo->hDC, &rcIm, CreateSolidBrush(RGB(255, 0, 100)));
        }
        break;
    case WM_DESTROY:
        PostQuitMessage(0);
